...
local SAVE_DIR = "Saves/roguelike/"
...
local function CallListeners(event, ...)
	...
	for i = 1, #eventListeners do
		...
		xpcall(function() listener(listener, unpack(args)) end,
			function(err) Spring.Echo("Roguelike Listener Error", err) end )
	end
	return true
end
...
local function WillOverrideSave(fileName)
	...
	if saveData.roguelikeID == gamedata.roguelikeID and (saveData.commanderName == gamedata.commanderName or saveData.commanderName == DEFAULT_COMM_NAME) then
		return false
	end
	return true
end
...
local function SaveGame()
	local fileName = WG.Chobby.Configuration.roguelikeSaveFile
	if (not fileName) or WillOverrideSave(fileName) then
		local number = WG.Chobby.Configuration.nextRoguelikeSaveNumber or 1
		for i = 1, 10000 do
		...
		WG.Chobby.Configuration:SetConfigValue("nextRoguelikeSaveNumber", number + 1)
		WG.Chobby.Configuration:SetConfigValue("roguelikeSaveFile", fileName)
	end
	local success, err = pcall(function()
		Spring.CreateDir(SAVE_DIR)
		...
		Spring.Echo("Roguelike Data", "Saved game to " .. path)
	end)
	if (not success) then
		Spring.Log(widget:GetInfo().name, LOG.ERROR, "Error saving game: " .. err)
	end
	return success, WG.Chobby.Configuration.roguelikeSaveFile
end
...
local function GenerateRoguelikeID()
	gamedata.roguelikeID = tostring(math.random())
end
...
local function LoadGame(saveData, refreshGUI)
	local success, err = pcall(function()
		...
		WG.Chobby.Configuration:SetConfigValue("roguelikeSaveFile", saveData.name)
		SaveGame()
		if refreshGUI then
			CallListeners("RoguelikeLoaded")
		end

		Spring.Echo("Roguelike Data", "Save file " .. saveData.name .. " loaded")
	end)
	if (not success) then
...
local function StartNewGame(overrideRoguelikeID)
	WG.Chobby.Configuration:SetConfigValue("roguelikeSaveFile", nil) -- Save name generated by campaign data handler
	--local roguelikeConfig = WG.Chobby.Configuration.roguelikeConfig
	ResetGamedata()
	
	if overrideRoguelikeID then
		gamedata.roguelikeID = overrideRoguelikeID
	else
		GenerateRoguelikeID()
	end

    --TODO: fix initial unlocks
...
local function SetupNewSave(commName, difficulty, overrideRoguelikeID)
	WG.RoguelikeData.SetCommanderName(commName)
	WG.RoguelikeData.SetDifficultySetting(difficulty)
	WG.RoguelikeData.SetRoguelikeInitializationComplete()
...
local function LoadRoguelikeData()
	Spring.Echo("Roguelike Data", "Loading roguelike data")
	local Configuration = WG.Chobby.Configuration
	...
	if Configuration.roguelikeSaveFile then
		...
		Spring.Echo("Roguelike Data", "No configured save data")
	end
	...
	for savename,saveData in pairs(saves) do
		WG.Chobby.Configuration:SetConfigValue("roguelikeSaveFile", savename)
		LoadGame(saveData, true)
		return true
	end

	-- we got nothing
	WG.Chobby.Configuration:SetConfigValue("roguelikeSaveFile", nil)
end
...
local function FindMatchingSave(roguelikeID, commName)
	-- Check whether the save is already loaded
	if roguelikeID == gamedata.roguelikeID and commName == gamedata.commanderName then
		return false
	end

	local saveFiles = GetSaves()
	for name, save in pairs(saveFiles) do
		if roguelikeID == save.roguelikeID and commName == save.commanderName then
			return true, name
		end
	end
...
function externalFunctions.SetDifficultySetting(newDifficulty)
	...
	CallListeners("RoguelikeSettingsUpdate")
	SaveGame()
end
...
function externalFunctions.GetRoguelikeInitializationComplete()
...
function externalFunctions.DeleteSave(filename, supressLastSavePrompt)
	...
	local success, err = pcall(function()
		...
		if (filename == Configuration.roguelikeSaveFile) then
			...
			WG.Chobby.Configuration:SetConfigValue("roguelikeSaveFile", newName)
			if not (newName or supressLastSavePrompt) then
				WG.RoguelikeSaveWindow.PromptPickNewSaveName()
			end
		end
	end)
	if (not success) then
...
function widget:Initialize()
	...
	WG.RoguelikeData = externalFunctions

	WG.Delay(LoadRoguelikeData, 0.1)
end

function widget:Shutdown()
	WG.RoguelikeData = nil
end
