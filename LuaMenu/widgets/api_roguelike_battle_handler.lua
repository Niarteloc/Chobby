...
function widget:GetInfo()
	return {
		name      = "Roguelike Battle Handler",
		desc      = "Handles creating the battle for roguelike missions as well as reporting results.",
		author    = "Niarteloc",
		date      = "30 Dec 2024",
...
local function StartBattleForReal(planetID, planetData)
	...
	local missionDifficulty = WG.RoguelikeData.GetDifficultySetting()
	local bitExtension = (Configuration:GetIsRunning64Bit() and "64") or "32"
	local playerName = Configuration:GetPlayerName()

	WG.Analytics.SendIndexedRepeatEvent("roguelike:planet_" .. planetID .. ":difficulty_" .. missionDifficulty .. ":started")
	...
	local playerUnlocks = WG.RoguelikeData.GetUnitsUnlocks()
	local playerAbilities = WG.RoguelikeData.GetAbilityUnlocks()

	commanderTypes.player_commander = WG.RoguelikeData.GetPlayerCommander()
	...
	teams[teamCount] = {
		...
		static_level = WG.RoguelikeData.GetPlayerCommanderInformation(),
		...
		retinuestartunits = TableToBase64(WG.RoguelikeData.GetActiveRetinue()),
	...
	for i = 1, #gameConfig.aiConfig do
		...
		local shortName = WG.RoguelikeData.GetAI(aiData.aiLib)
...
--------------------------------------------------------------------------------
-- External Functions
local RoguelikeBattleHandler = {}

function RoguelikeBattleHandler.StartBattle(planetID, planetData)
	local Configuration = WG.Chobby.Configuration
...
function widget:Initialize()
	WG.RoguelikeBattleHandler = RoguelikeBattleHandler
end
