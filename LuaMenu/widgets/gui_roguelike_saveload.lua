...
function widget:GetInfo()
	return {
		name      = "Roguelike Save/Load",
		desc      = "Create and manage roguelike saves",
		author    = "Niarteloc",
		date      = "30 Dec 2024",
...
local function SaveGame(filename)
	local result = WG.RoguelikeData.SaveGame(filename)
	if result and WG.RoguelikeSaveWindow.PopulateSaveList then
		WG.RoguelikeSaveWindow.PopulateSaveList()
	end
end

local function LoadGame(filename)
	WG.RoguelikeData.LoadGameByFilename(filename)
	if WG.RoguelikeSaveWindow.PopulateSaveList then
		WG.RoguelikeSaveWindow.PopulateSaveList()
	end
end

local function DeleteSave(filename, supressLastSavePrompt)
	WG.RoguelikeData.DeleteSave(filename, supressLastSavePrompt)
	if WG.RoguelikeSaveWindow.PopulateSaveList then
		WG.RoguelikeSaveWindow.PopulateSaveList()
	end
end
...
local function PromptNewSave(backOnFail)
	...
	local saveFile
	if backOnFail then
		saveFile = WG.RoguelikeData.StartNewGame()
	end
	...
	local function NewSave()
		if ebSaveName.text and ebSaveName.text ~= "" then
			if not saveFile then
				WG.RoguelikeData.StartNewGame()
			end
			WG.RoguelikeData.SetupNewSave(ebSaveName.text, comboDifficulty.selected)
...
local function AddSaveEntryButton(saveFile, saveList)
	local Configuration = WG.Chobby.Configuration
	local current = (saveFile.name == Configuration.roguelikeSaveFile)
...
local function UpdateSaveList(saveList)
	saveList:Clear()
	local saves = WG.RoguelikeData.GetSaves()
	local items = {}
...
local RoguelikeSaveWindow = {}

function RoguelikeSaveWindow.GetControl()
	local controlFuncs

	local window = Control:New {
		name = "roguelikeSaveHandler",
		x = "0%",
		y = "0%",
		width = "100%",
		height = "100%",
		OnParent = {
			function(obj)
				if obj:IsEmpty() then
					...
					RoguelikeSaveWindow.PopulateSaveList = controlFuncs.PopulateSaveList -- hax
				end
			end
		},
	}
...
function RoguelikeSaveWindow.PromptPickNewSaveName()
	local Configuration = WG.Chobby.Configuration
	if not Configuration.roguelikeSaveFile then
		PromptNewSave(true)
	end
end

function RoguelikeSaveWindow.PromptInitialSaveName()
	local Configuration = WG.Chobby.Configuration
	if not Configuration.roguelikeSaveFile then
		local lobby = WG.LibLobby.lobby
		...
		if commName then
			WG.RoguelikeData.StartNewGame()
			WG.RoguelikeData.SetupNewSave(commName, 2)
		else
			PromptNewSave(true)
		end
	end
...
function widget:Initialize()
	...
	WG.RoguelikeSaveWindow = RoguelikeSaveWindow
end

function widget:Shutdown()
	WG.RoguelikeSaveWindow = nil
end
